...
class FrankaGrasp(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # create a stick as a hammer
        box_dims = gymapi.Vec3(0.35, 0.03, 0.03)
        asset_options = gymapi.AssetOptions()
        hammer_asset = self.gym.create_box(self.sim, box_dims.x, box_dims.y, box_dims.z, asset_options)
...
def compute_franka_reward2(
    ...
    both_sides_weight = 1.0
    lift_weight = 4.0
    finger_dist_weight = 10.0
    ...
    # distance from hand to the hammer
    hammer_pos = hammer_pose[:, :3]
    d = torch.norm(franka_gripper_pos - hammer_pos, p=2, dim=-1)
    dist_reward = 1.0 / (1.0 + d ** 2)
    dist_reward = torch.where(d <= 0.02, dist_reward *2, dist_reward)

    # reward for matching the orientation of the hand to the hammer
    hammer_rot = hammer_pose[:, 3:7]
    y_dir = to_torch([0, 1, 0]).repeat(num_envs, 1)
    hammer_y_dir = quat_apply(hammer_rot, y_dir)
    gripper_y_dir = quat_apply(gripper_rot, y_dir)
    ...
    rot_reward = 0.5 * (torch.sign(gripper_hammer_y_dot) * gripper_hammer_y_dot ** 2 + torch.sign(gripper_z_dot) * gripper_z_dot ** 2)

    # bonus if two fingers are on both sides of the hammer
    hammer_to_lfinger = franka_lfinger_pos - hammer_pos
    hammer_to_lfinger_dist = torch.norm(hammer_to_lfinger, dim=-1).unsqueeze(-1)
    hammer_to_lfinger_dir = hammer_to_lfinger / hammer_to_lfinger_dist
    ...
    hammer_both_sides_dot = torch.bmm(hammer_to_lfinger_dir.view(num_envs, 1, 3), hammer_to_rfinger_dir.view(num_envs, 3, 1))
    both_sides_reward = -torch.sign(hammer_both_sides_dot) * hammer_both_sides_dot ** 2

    finger_dist_reward = torch.zeros_like(rot_reward)
    finger_dist_reward = torch.where(hammer_both_sides_dot < -0.3, (0.04 - hammer_to_lfinger_dist) + (0.04 - hammer_to_rfinger_dist), finger_dist_reward)

    # regularization on the actions (summed for each environment)
    action_penalty = torch.sum(actions ** 2, dim=-1)

    # how far the hammer is lifted
    lift_reward = hammer_pos[:, 2] - 0.4

    rewards = dist_reward_weight * dist_reward + rot_reward_weight * rot_reward \
              + both_sides_weight * both_sides_reward + lift_weight * lift_reward \
              + finger_dist_weight * finger_dist_reward - action_penalty_scale * action_penalty

    rewards = torch.where(lift_reward > 0.38, rewards + 1.0,
                          torch.where(lift_reward > 0.2, rewards + 0.8,
                                      torch.where(lift_reward > 0.15, rewards + 0.65,
                                                  torch.where(lift_reward > 0.1, rewards + 0.5,
                                                              torch.where(lift_reward > 0.05, rewards + 0.35,
                                                                          torch.where(lift_reward > 0.01,
                                                                                      rewards + 0.25,
                                                                                      torch.where(lift_reward > 0.0,
                                                                                                  rewards + 0.15,
                                                                                                  rewards)))))))
