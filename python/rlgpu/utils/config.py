...
def retrieve_cfg(args, use_rlg_config=False):
    if use_rlg_config:
        ...
        if args.task == "BallBalance":
            return os.path.join(args.logdir, "ball_balance"), "cfg/train/pytorch_ppo_ball_balance.yaml", "cfg/ball_balance.yaml"
        elif args.task == "Cartpole":
            return os.path.join(args.logdir, "cartpole"), "cfg/train/pytorch_ppo_cartpole.yaml", "cfg/cartpole.yaml"
        elif args.task == "CartpoleYUp":
            return os.path.join(args.logdir, "cartpole_y_up"), "cfg/train/pytorch_ppo_cartpole.yaml", "cfg/cartpole_y_up.yaml"
        elif args.task == "Ant":
...
def load_cfg(args, use_rlg_config=False):
    ...
    if use_rlg_config:
        ...
        if args.experiment_name != 'Base':
            exp_name = "{}_{}_{}_{}".format(args.experiment_name, args.task_type, args.device, str(args.physics_engine).split("_")[-1])
        ...
        log_id = args.logdir
        if args.experiment_name != 'Base':
            log_id = args.logdir + "_{}_{}_{}_{}".format(args.experiment_name, args.task_type, args.device, str(args.physics_engine).split("_")[-1])
            if cfg["task"]["randomize"]:
                log_id += "_DR"
...
def get_args(benchmark=False, use_rlg_config=False):
    custom_parameters = [
        ...
        {"name": "--task", "type": str, "default": "Humanoid",
            "help": "Can be Ant, BallBalance, Cartpole, CartpoleYUp, Humanoid, FrankaCabinet, ShadowHand"},
